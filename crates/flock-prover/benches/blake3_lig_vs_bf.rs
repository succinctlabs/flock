//! BLAKE3 `prove_fast` (BaseFold) vs `prove_fast_ligerito` head-to-head.
//!
//! Run twice for ST and MT:
//!   cargo bench --bench blake3_lig_vs_bf                              # MT (default)
//!   RAYON_NUM_THREADS=1 cargo bench --bench blake3_lig_vs_bf          # ST
//!
//! By default benches at m=30 (K=65536 BLAKE3 compressions). Override with
//! `BLAKE3_K=<n_blocks>`; the bench picks the smallest m ≥ 21 that fits the
//! Ligerito default config (log_batch_size=6).
//!
//! For each backend prints best-of-N prove time, peak memory, verify time,
//! and serialized proof size.
...
// Peak-heap tracker (mirrors keccak_proof.rs).
struct PeakAlloc;
...
fn bench_block(n_blocks: usize, n_runs: usize, threads_label: &str) {
    ...
    // ============ BaseFold ============
    {
        ...
        let (p, _, _) = setup.prove_fast_basefold(&block_sets[0], &mut ch_p);
        black_box(&p);
        ...
        for run in 0..n_runs {
            let mut ch_p = FsChallenger::new(b"flock-bench-v0");
            let t0 = Instant::now();
            let (p, _, _) = setup.prove_fast_basefold(&block_sets[run + 1], &mut ch_p);
        ...
        reset_peak();
        let mut ch_p = FsChallenger::new(b"flock-bench-v0");
        let (proof, commitment, _) = setup.prove_fast_basefold(&block_sets[0], &mut ch_p);
...
fn main() {
    ...
    // Default K=65536 → m=30. Override with BLAKE3_K=<value>.
