//! SHA-256 `prove_fast` (BaseFold) vs `prove_fast_ligerito` head-to-head.
//!
//! Run twice for ST and MT:
//!   cargo bench --bench sha2_lig_vs_bf                              # MT (default)
//!   RAYON_NUM_THREADS=1 cargo bench --bench sha2_lig_vs_bf          # ST
//!
//! Default K=32768 (= 2^15 → m=30 with K_LOG=15). Override with SHA2_K=<n>.
...
use flock_prover::r1cs_hashes::sha2::{K_LOG, Sha256HybridSetup, min_n_blocks_log};
...
fn bench_block(n: usize, n_runs: usize, threads_label: &str) {
    ...
    println!(
        "\n=== K = {n:>6} compressions  (m = {m}, slots = {n_slots}, witness = {} MB, {threads_label}) ===",
        witness_bytes >> 20
    );
    ...
    let mk_inputs = |seed: u64| {
        ...
        (0..n)
            .map(|_| (SHA256_IV, rng.next_block()))
            .collect::<Vec<([u32; 8], [u32; 16])>>()
    };
    let input_sets: Vec<Vec<([u32; 8], [u32; 16])>> = (0..=n_runs)
        .map(|run| mk_inputs(0x5_a256_be3f ^ (n as u64) ^ (run as u64)))
        .collect();
...
fn main() {
    ...
    println!(
        "SHA-256 (K_LOG={}) prove_fast (BaseFold) vs prove_fast_ligerito — {}",
        K_LOG, label
    );
    ...
    let ks: Vec<usize> = match std::env::var("SHA2_K") {
        Ok(s) => s
            ...
            .filter(|t| !t.is_empty())
            .map(|t| t.parse().expect("SHA2_K: integer K"))
            .collect(),
        Err(_) => vec![32768], // 2^15 → m = 30
    };
