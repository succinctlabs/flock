...
//! `src/r1cs_hashes/mul64_karatsuba.rs` (subtractive Karatsuba, K_LOG=17,
//! 25 products per block at ~99% fill).
...
use flock_prover::r1cs_hashes::mul64_karatsuba::{
    K, K_LOG, Mul64KaratsubaSetup as Mul64Setup, circuit, min_n_blocks_log, muls_per_block,
};
...
fn bench_one(n_muls: usize, n_runs: usize) {
    ...
    let total_useful_bits = (n_muls * circuit().sub_bits) as f64;
    ...
    println!(
        "  variant: {}  bits/mul {}  nnz/mul {}  muls/blk {}",
        std::env::var("MUL64K_SUB").unwrap_or_else(|_| "additive".into()),
        circuit().sub_bits,
        circuit().nnz / circuit().muls_per_block.max(1),
...
fn main() {
    ...
    let c = circuit();
    println!(
        "u64×u64 batch multiplication R1CS, subtractive Karatsuba (K_LOG={K_LOG}, {} \
         products/block, {}/{K} = {:.2}% fill).\n\
         Per product: {} committed bits (stride {}); A₀/B₀ nnz = {}/product.",
        c.muls_per_block,
        c.useful_bits,
    ...
    // counts, larger values are raw counts — muls_per_block()·2^k counts give
    // 100% slot fill).
    let mpb = muls_per_block();
