...
/// Informational: what the wiring layer COSTS on a real shape — a ν = 10
/// SHA-256 tree (256 leaves → 511 gates, μ = 14). Prints the wiring phases
/// against the whole prove, and the claim count. Reported, not optimized.
///
/// Run with `cargo test --release -p flock-prover --test circuit_wiring --
/// --ignored wiring_cost_probe -- --nocapture`.
#[test]
#[ignore] // Informational cost probe.
fn wiring_cost_probe() {
    let (nu, k_leaves) = (10usize, 8usize); // 256 leaves → 511 gates
    let (registry, r1cs) = sha2_registry(nu);
    let mut rng = Rng::new(0x5A20_C057);
    let tree = build_tree(k_leaves, nu, &mut rng);
    let union = UnionInstance::new(&registry, vec![tree.n_gates]);
    let pcs_params = union_pcs_params(&union);
    let circuit = Circuit::new(
    ...
    // The wiring layer alone, timed against the same buffers the prove uses.
    let z_packed = {
        let (z, _, _, _) = sha2::generate_witness_batch_major_partial(&tree.compressions, nu);
        z
    };
    let mut ch = FsChallenger::new(DOMAIN);
    let t = Instant::now();
    ...
    let gkr_bytes = bincode::serialize(&proof.wiring).expect("serialize").len();
    let all_bytes = bincode::serialize(&proof).expect("serialize").len();
    println!(
        "[wiring cost] ν={nu} gates={} μ={} cells={} | prove(wiring GKR + gather) {wiring_ms:.2} ms \
         of {prove_ms:.2} ms total | verify {verify_ms:.2} ms | gather claims {} | \
         wiring proof {gkr_bytes} B of {all_bytes} B",
        tree.n_gates,
        cells.mu(),
        1usize << cells.mu(),
        claims.len(),
