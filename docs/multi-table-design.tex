...
types, but undeclared capacity is address space, not work
(\S\ref{sec:zc-sparse}), and the round count exceeds that of an
...
of \S\ref{sec:union} exists purely virtually. The pipeline is best
...
\begin{enumerate}[nosep]
  ...
  \item the \emph{dense packed polynomial} $\hat q$
        ...
        with the dead words deleted --- the only polynomial Ligerito
        commits and opens.
\end{enumerate}
...
under the declared-work discipline (a Phase 2 item --- the Phase 1
...
\paragraph{Reduction 2: the jagged transport.} Stack only the \emph{useful} columns of the
...
the pipeline. Under the uniform-capacity convention the identification is
exact: a packed word's address is $[\,\nu \text{ batch bits} \mid
\text{column bits}\,]$ uniformly across slots, chunk-columns enumerate in
address order, and $\hat f$ \emph{is} the jagged grid polynomial
($n = \nu$, $k = M - 7 - \nu$, heights $n_t$ words on used chunk-columns,
$0$ elsewhere). Commit the dense stack with Ligerito, and transport the
single claim $\hat f(\rho) = v_\rho$ via the jagged sumcheck with assist
(\code{pcs/jagged.rs}): $z_{\mathrm{row}} = \rho[0{:}\nu]$,
$z_{\mathrm{col}} = \rho[\nu{:}]$, with \code{col\_prefix\_sums} ---
derived per proof from the public counts --- encoding the layout, and the
branching-program evaluator $\hat f_t$, with the ``assist'' delegation,
keeping the verifier cheap. The output is one dense claim
$\hat q(\rho') = \alpha$, which Ligerito opens directly. The region
...
One sizing caveat: the committed length must be a power of two, and at
single-table full utilization the dense stack rounds straight back to the
padded length (BLAKE3 uses $121$ of $128$ chunk-columns, SHA-256 $246$ of
$256$) --- area savings materialize only for mixed and partial-utilization
instances, whose totals land between powers of two.
...
Jagged transport prover & $\tilde O(N_{\mathrm{com}})$ (single-claim transport SC $+$ assist) \\
...
\begin{description}[leftmargin=1.6em, style=nextline]
  ...
  \item[Phase 2 --- the union PIOP and dynamic counts (mixed proofs ship).]
    ...
    committed-area saving realized on mixed and partial-utilization
    instances.
\end{description}
...
\begin{enumerate}[nosep]
  \item \textbf{Phase 1 wiring constraints.} Claim batching is fully
        resolved by architecture (\S\ref{sec:commit}: it lives on
        reduction 1, in existing $\gamma$-machinery; the transport always
        carries one claim). What remains is wiring: a Ligerito security
        configuration must exist for each dense size the registry can
        produce (enumerable at registry-definition time; at single-table
        full utilization the dense size equals today's, so Phase 1 needs
        no new configurations); the virtual-opening sumcheck needs its
        domain label and transcript position pinned; and the ring-switch
        \code{ChunkPadding} multi-run fallback (currently no-skip,
        correct but unoptimized) is generalized alongside the folding
        item above.
