...
\large Design document --- working draft v0.4}
\author{Flock}
\date{July 23, 2026}
...
The committed size is this rounded up to a power of two,
\textbf{floored} at the smallest embedded Ligerito security
configuration ($m22$: $2^{15}$ packed words --- configurations are
derived and embedded per committed size) and capped at the union's own
padded size, so the dense domain never exceeds the virtual one. The
...
evaluator $\hat f_t$.
The sizing fact, sharpened by implementation: above the $m22$ config
floor the committed size is \textbf{count-proportional} --- the
power-of-two rounding costs at most $2\times$, and the floor binds only
when the dense area falls below $2^{15}$ packed words. (The
capacity-height column-dense rounding effect of the previous draft ---
column-dense registries like BLAKE3$+$SHA-256, at $121$ of $128$ and
$246$ of $256$ chunk-columns, rounding straight back to the padded power
of two --- survives only at full utilization, where the dense area is
the used-column capacity area.)
...
after rounding up to a power of two and flooring at the $m22$ config,
\S\ref{sec:commit}), $T$ = types.
...
$m22$ config floor under height-$n_t$ stacking (\S\ref{sec:commit}),
with the power-of-two rounding costing at most $2\times$; the
column-dense rounding effect (the useful-column ratio returned by the
rounding) survives only at full utilization.
...
(Remark~\ref{rem:harness}) remains a live regression anchor. Height-$n_t$
stacking (M5) has since landed, making the committed size
...
CLI's mixed proof of \code{blake3=100,sha2=37} is $\approx 274$\,KB,
proving and verifying in $\approx 0.3$\,s each.

Open follow-ups, in the order they are likely to matter:
