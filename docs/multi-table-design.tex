...
needs no union-awareness. The jagged transport therefore operates purely
...
\begin{enumerate}[nosep]
  \item \textbf{Jagged claim batching (Phase 1).} The jagged kernel's
        ...
        layer must add is claim \emph{batching}: many per-slot packed
        claims, at different points, RLC-combined into one transport
        sumcheck (combine the per-claim $\eq$-weight tables under random
        coefficients). Bounded, known work --- design at the start of
        Phase 1.
