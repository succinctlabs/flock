...
$0$ elsewhere). Commit the dense stack with Ligerito. The transport itself is
\emph{fused into the opening}: since $\hat f(\rho) = \sum_e q[e]\,
W_\rho(e)$ with $W_\rho(e) = \eq(\rho[0{:}\nu], \mathrm{row}(e))\cdot
\eq(\rho[\nu{:}], \mathrm{col}(e))$ --- the region beyond height $n_t$
is zero \emph{by definition}, which is what discharged the soundness
obligation of Remark~\ref{rem:padding} --- Ligerito, itself an
...
$yr\_log\_n/|\F|$. \code{col\_prefix\_sums}, derived per proof from the
public counts, encodes the layout inside the branching-program evaluator
$\hat f_t$.
