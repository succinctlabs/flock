...
invocations, heights $= n_t$ \emph{words}. Under the uniform-capacity
convention this is exact, not analogical: a packed word's address is
$[\,\nu \text{ batch bits} \mid \text{column bits}\,]$ uniformly across
slots, the chunk-columns enumerate in address order, and the packed union
polynomial \emph{is} the jagged grid polynomial ($n = \nu$,
$k = M - 7 - \nu$, heights $n_t$ on used chunk-columns, $0$ elsewhere).
Union-level claims therefore transport directly --- no per-slot
decomposition. Better still, the two core claims share their row point
($z_{\mathrm{row}} =$ the packed image of $r|_R$ for both), so the
batched transport weight is one row-$\eq$ table times a two-term column
table.
...
\begin{enumerate}[nosep]
  \item \textbf{Jagged claim batching (Phase 1) --- now small.} The
        jagged kernel's public API (branch \code{jagged-assist})
        transports exactly one sparse claim per invocation, and
        ring-switching runs first, so the transport only ever sees
        ordinary multilinear points over packed-word variables ---
        \emph{no point-format adapter is needed}. The proof emits just
        two core claims sharing their row point (\S\ref{sec:commit}),
        plus glue on single-type statements, so the batched transport
        weight is one row-$\eq$ table times a few column terms,
        RLC-combined under transcript coefficients --- the same
        multi-claim front-end \code{open\_batch\_mixed} already provides
        at the Ligerito layer. A per-claim transport v0 is sound and an
        acceptable stepping stone inside Phase 1.
