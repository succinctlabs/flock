...
\]
This is today's \emph{row-major} convention (one invocation's trace is
contiguous); the multi-table design deliberately transposes it
(\S\ref{sec:union}).
...
``glue'', \emph{not} extra R1CS rows) --- are batched by the existing
machinery (\code{pcs::open\_batch\_mixed\ldots}, label
...
\paragraph{Layout convention (BatchMajor).} Within a slot, indices are
column-major: the low $\nu_t^{\max}$ bits are the row (invocation) index
and the next $\kappa_t$ bits the column (trace-variable) index, so each
trace variable's column is contiguous across invocations. This transposes
the single-table recap's row-major convention (\S\ref{sec:single}), and
the choice is deliberate: it is the stacking order the jagged commitment
needs (\S\ref{sec:commit}); it makes sparse constraint application
\emph{bit-sliced} --- each nonzero of $A_t$ becomes a contiguous
column-XOR, vectorized across invocations --- and \S\ref{sec:skip} shows
the univariate skip transfers intact. Witness generation, which naturally
emits one invocation (row) at a time, writes this layout via cache-tiled
bit-transposes, an existing kernel.
...
\[
  x[0{:}\nu_t^{\max}] = \text{row index},\quad
  x[\nu_t^{\max}{:}m_t] = \text{column index},\quad
...
\subsection{The univariate skip survives globally --- over row
bits}\label{sec:skip}
...
domain points be memory-contiguous. \emph{Which} six variables are merged
is a free protocol choice. Under the BatchMajor convention, the global low
six bits of any index are within-column \emph{row} bits for every slot
(all $\nu_t^{\max} \ge 6$ in any real registry, and every slot offset is a
multiple of $2^{m_t}$), so the single global skip round runs over the six
low row bits: a skip group is the same trace variable across $64$
consecutive invocations --- in packed representation, \textbf{exactly one
64-bit machine word of a column}, the natural unit of the bit-sliced
kernels. The zerocheck constraint is pointwise, so nothing else changes:
one skip round, all slots at once, no per-table scheduling. The skip's
univariate coordinate consequently sits among the \emph{row} coordinates
of the zerocheck's output point; \S\ref{sec:lc} tracks where that lands
(the witness folds inherit the quirky weight table, while the comb side
becomes a standard $\eq$ fold), and the claim pipeline must accept such
points --- a Phase 1 validation item (\S\ref{sec:open}).
...
\begin{itemize}[nosep]
  ...
  \item The zerocheck sumcheck algorithm: \textbf{unchanged} (it is
        pointwise and block-agnostic; the code map confirmed the uniform
        \code{PaddingSpec} was its only single-table assumption). The
        fold/skip kernels are re-targeted to the BatchMajor orientation
        (skip groups become column words, \S\ref{sec:skip}) --- same
        shape, new memory order.
\end{itemize}
...
$\eq(r,x) = \eq(r[m_t{:}M], p_t)\cdot\eq(r[\nu_t^{\max}{:}m_t],
x_{\mathrm{col}})\cdot\eq(r[0{:}\nu_t^{\max}], x_{\mathrm{row}})$, and the
prefix factor ---
...
identity contributes $\eq(r[0{:}\nu_t^{\max}], c[0{:}\nu_t^{\max}])$
(``same invocation'') via
\begin{equation*}
  \sum_{w \in \{0,1\}^{\nu}} \eq(a, w)\,\eq(b, w) \;=\; \eq(a, b),
\end{equation*}
and the base block contributes $\hat A_t$. Note each term splits the
\emph{same} shared coordinates at its own boundaries $\nu_t^{\max}, m_t$
...
collapsing, per slot, the sum over the row coordinates
$c[0{:}\nu_t^{\max}]$ into a partial evaluation of that slot's witness MLE
(exactly as in the single-table case):
\begin{equation}\label{eq:lc-claim}
  ...
  \begin{aligned}
    ...
    y_t(c') &= \hat z_t\bigl(r[0{:}\nu_t^{\max}],\, c'\bigr),
  \end{aligned}
\end{equation}
where
$\mathrm{comb}^A_t(c') = \sum_{x'} \eq(r[\nu_t^{\max}{:}m_t], x')\,
A_t[x', c']$
is the base block folded by the \emph{standard} $\eq$ table of the
within-block constraint coordinates of $r$ (computed through the type's
\code{LincheckCircuit}), and $\hat z_t(\cdot,\cdot)$ is the MLE of slot
$t$'s (zero-padded) witness, rows first. The prover materializes each
$y_t$ by folding slot $t$'s \emph{declared rows} with the weight table of
the row coordinates $r[0{:}\nu_t^{\max}]$ --- the \emph{quirky} table,
since these coordinates contain the univariate skip coordinate
(\S\ref{sec:skip}): Lagrange-at-$z$ weights for the low six bits times
$\eq$ for the rest, at the same fold cost as a standard table ---
$O(\sum_t n_t k_t)$ in total, the declared area, analogous to today's
single outer fold. (Note the division of labor relative to today's code:
the skip structure has migrated from the comb side, now a plain $\eq$
fold, to the witness-fold side.)
...
$y_t(c^*_t) = \hat z_t(r[0{:}\nu_t^{\max}], c^*_t)$ --- one witness claim
per type.
...
this): the committed size is $\sum_t n_t u_t$, with all columns of slot $t$
having height exactly $n_t$ --- non-power-of-two counts are native.
Because the union's virtual layout is itself BatchMajor
(\S\ref{sec:union}), the committed stack is simply the union buffer with
dummy rows and useless columns omitted --- commitment and address space
share one orientation, with no transpose anywhere in the pipeline. Commit
...
\begin{description}[leftmargin=1.6em, style=nextline]
  ...
  \item[Phase 1 --- jagged under the existing single-table system.]
    ...
    (one-type registry): same proofs, new commitment path, so every
    existing test and benchmark is a correctness and throughput oracle.
    Validate the BatchMajor-native design here, by measurement: the
    row-bit skip kernels (skip group $=$ one packed \code{u64} of a
    column, \S\ref{sec:skip}), the bit-sliced constraint application, and
    the claim pipeline's handling of univariate-coordinate-in-row points
    --- the design's one throughput-critical unknown, moved to the front.
    Gate: single-hash throughput within an agreed margin of the direct
    path; useful-column area saving realized.
...
\begin{enumerate}[nosep]
  \item \textbf{BatchMajor-native skip (Phase 1).} The design adopts the
        BatchMajor layout with the univariate skip over the six low
        \emph{row} bits (\S\ref{sec:skip}) --- single buffer from witness
        generation through commit through PIOP. Hypotheses to validate by
        measurement: (a) vertical-group skip kernels (one packed
        \code{u64} per group) match or beat today's horizontal-group
        kernels; (b) the packed-direct / ring-switch claim pipeline
        accommodates evaluation points whose univariate coordinate sits
        among the row coordinates. Fallback if either fails: dual buffers
        --- a row-major working copy for the PIOP, BatchMajor for commit
        --- via the existing tiled bit-transpose, at one extra pass and
        $2\times$ witness memory.
  ...
  \item \textbf{Folding over empty capacity --- an implementation
        ...
        sparse iteration inside any hot loop. Residual work: the
        round-polynomial accounting at slot-collapse boundaries, and an
        adversarial benchmark at low utilization.
