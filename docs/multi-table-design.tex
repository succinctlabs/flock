...
conventions carry over; the one place the coordinate still needs an
adapter is the jagged transport's point format (\S\ref{sec:open}).
...
\begin{description}[leftmargin=1.6em, style=nextline]
  ...
  \item[Phase 1 --- jagged under the existing single-table system.]
    ...
    layer: multi-claim RLC batching into one jagged transport, and the
    point-format adapter for the univariate skip coordinate
    (\S\ref{sec:open}). Gate: single-hash throughput within an agreed
    margin of the direct path; chunk-granular area saving realized.
...
\begin{enumerate}[nosep]
  \item \textbf{Jagged composition adapters (Phase 1).} The jagged
        kernel's public API (branch \code{jagged-assist}) transports
        exactly one sparse claim and takes its evaluation point as plain
        multilinear $(z_{\mathrm{row}}, z_{\mathrm{col}},
        z_{\mathrm{index}})$ slices. The composition layer must add:
        (a) RLC batching of many per-slot claims into one transport
        sumcheck; (b) an adapter for the univariate skip coordinate,
        which sits among the coordinates jagged treats as ordinary
        multilinear --- either extend the transport's weight tables to
        accept the quirky (Lagrange-at-$z$) weights for those six bits,
        or pre-reduce the quirky claim to a multilinear claim with a
        six-round mini-sumcheck. Both are bounded, known work items ---
        design them at the start of Phase 1.
