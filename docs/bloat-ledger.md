...
- Kept: the unfused `round1_straight_line_program_packed_banks` kernel (differential oracle),
...
removal). The k-ary arity paths (`build_first_level_node_k` /
...
| --- | --- | --- | --- | --- |
| 1 | tower.rs Real/Child tape+region+node family (walkers share 76%, emitters 78%) | 2,488 | ~2,000 | **high** — transcript + byte pins; unify field-by-field, re-run pins **2026-09-03 re-scoped after measurement (Ron)**: LOC is not the point — measured 50% line-identical after name canonicalization (new 0.58 / emit 0.56 / check 0.22 similarity), and the walkers parse DIFFERENT inners (MixedInner leaf vs LeafOuter envelope), so a parameterized mega-walker would be flag soup. Revised plan: A0 pin the REAL tape (DONE, `real_tape_regions_pinned`), A section-split both monoliths along their `// ----` markers (pure motion, byte-pinned), B share ONLY the byte-identical format-defined sections (chain materials, spine quad replay, H(publics), recombination, jagged emission, ordinal walking), NO WalkSide parameterization, `check_*` never merged. The real fix is a **declarative op-schedule/region table** both walkers and the pins derive from — the same conclusion as cluster 9 — sequenced with tower productionization, not this campaign. **A+B EXECUTED 2026-09-03**: A0 real-side pin + section split (13+8 / 8+10 named helpers, byte-pinned); B measurement post-split found NO whole section byte-identical (best pair 0.82) — the shareable truth is sub-blocks, extracted as `walker_common.rs` (8 wire-format walks, ~430 shared lines: the GKR transcript walk, chain merge/replay + level census, the spine walk, family-H sources, three anchor-expect waists); genuinely protocol-different pieces left per-walker on purpose. Cluster CLOSED; the op-schedule table remains the endgame. |
